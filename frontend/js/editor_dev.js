...
const els = {
  ...
  bgSvgContainer: document.getElementById("bgSvgContainer"),
...
function updateStageDisplaySize() {
  ...
  if (els.stage) {
    els.stage.style.width = `${disp.width}px`;
    els.stage.style.height = `${disp.height}px`;
  }
  if (els.bgSvgContainer) {
    els.bgSvgContainer.style.width = "100%";
    els.bgSvgContainer.style.height = "100%";
  }
  if (els.overlay) {
...
function renderInlineSvgBackground() {
  const container = els.bgSvgContainer;
  if (!container) return;
  const floor = currentFloor();
...
function updateBgOpacityControls(opacity) {
  ...
  if (els.bgSvgContainer) {
    els.bgSvgContainer.style.opacity = clamped;
  }
}
...
function renderFloor() {
  ...
  if (url) {
    ...
    els.bgImg.style.display = "none";
    if (els.bgSvgContainer) {
      els.bgSvgContainer.innerHTML = "";
      els.bgSvgContainer.style.display = "none";
    }
    if (els.canvasFrame) {
...
function applyViewTransform() {
  ...
  const viewHeight = size.height / scale;
  const clamp = (val, min, max) => Math.max(min, Math.min(max, val));
  const maxX = Math.max(0, size.width - viewWidth);
  const maxY = Math.max(0, size.height - viewHeight);
  state.view.tx = clamp(Number(state.view?.tx) || 0, 0, maxX);
  state.view.ty = clamp(Number(state.view?.ty) || 0, 0, maxY);
  if (els.stage) {
    els.stage.style.transform = `translate(0px, 0px)`;
  }
  if (els.overlay) {
  ...
  const bgSvg = els.bgSvgContainer?.querySelector("svg");
  if (bgSvg) {
    bgSvg.setAttribute(
      "viewBox",
      `${state.view.tx} ${state.view.ty} ${viewWidth} ${viewHeight}`
